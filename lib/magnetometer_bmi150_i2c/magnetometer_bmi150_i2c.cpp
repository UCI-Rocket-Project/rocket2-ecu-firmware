...
MagBmi150i2c::MagBmi150i2c(I2C_HandleTypeDef *hi2c,
                           GPIO_TypeDef *intPort, uint16_t intPin,
                           GPIO_TypeDef *drdyPort, uint16_t drdyPin)
    : _hi2c(hi2c),
      _intPort(intPort), _intPin(intPin),
      _drdyPort(drdyPort), _drdyPin(drdyPin)
{
}

/* Notes */
// Any array in camelCase is an array that the device outputs to
// Any array in UPPER_SNAKE_CASE functions as a constant variable and is the input hex the device uses as an input
...
MagBmi150i2c::Data MagBmi150i2c::Read()
{
    ...
    if (rawZData != -16384)
    {
        if ((dig_Z2 != 0) && (dig_Z1 != 0) && (rawRHALLData != 0) &&
            (dig_XYZ1 != 0))
        {
            /*Processing compensation equations */
