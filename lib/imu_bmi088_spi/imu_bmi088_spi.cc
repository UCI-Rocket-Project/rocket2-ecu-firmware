...
int ImuBmi088Spi::Init() {
    ...
    // Dummy byte 0x00
    uint8_t ACC_CHIP_ID[2] = {0x00 | 0x80};
    uint8_t accID[2] = {0};

    HAL_GPIO_WritePin(_accCsPort, _accCsPin, GPIO_PIN_RESET);
    if (HAL_SPI_Transmit(_hspi, ACC_CHIP_ID, 1, SERIAL_TIMEOUT) != HAL_OK) return 1;
    if (HAL_SPI_Receive(_hspi, accID, 2, SERIAL_TIMEOUT) != HAL_OK) return 1;
    HAL_GPIO_WritePin(_accCsPort, _accCsPin, GPIO_PIN_SET);
